...
static void AppTask3(void *p_arg)
{
  ...
  printf("key1count=%d\n",key1count);
  printf("KEY1按键值1~6对应不同任务情况，KEY2确定\n");
    ...
    if(Key_AccessTimes(&key2,KEY_ACCESS_READ)!=0)//按键被按下过
    {
      switch(key1count)
      {
        case 1:
          ...
          break; 
        case 2:
          OSTaskSuspend(APP_TASK1_PRIO);		    /* 挂起优先级是4的任务，挂起自己是OS_PRIO_SELF */
          printf("挂起任务1\n");
          break;        
        case 3:
          OSTaskResume(APP_TASK1_PRIO);		    /* 恢复此任务 */
          printf("恢复任务1\n");
          break;          
        case 4:
          err = OSTaskDelReq(APP_TASK2_PRIO);     /* 请求AppTaskCom删除自己 */ 
         	if (err == OS_ERR_NONE) 
          { 
            while (err != OS_ERR_TASK_NOT_EXIST) 
            { 
               err = OSTaskDelReq(APP_TASK2_PRIO); 
               OSTimeDly(30);                      /* 等待被删除  */ 
             } 
          }	 
          printf("删除任务2\n");
        ...
        case 5:                       
          ...
          break; 
        case 6:
          OSTaskChangePrio(APP_TASK1_PRIO,10);    /* 10: 新修改的优先级 APP_TASK1_PRIO: 原来的优先级 */
          printf("修改任务1的优先级\n");
          break;                              
      }
