...
static void AppTask1(void *p_arg)
{
	...
	{	
		...
		printf("接受到按键Key1和按键Key2都被按下过的消息\r\n");
	}   
}
...
static void AppTask3(void *p_arg)
{
  ...
  (void)p_arg;  
    ...
    if(Key_AccessTimes(&key1,KEY_ACCESS_READ)!=0)/* Key1键按下 设置标志位 bit0 */
    {
    ...
    if(Key_AccessTimes(&key2,KEY_ACCESS_READ)!=0)/* Key2键按下 设置标志位 bit1 */
    {
     OSFlagPost ((OS_FLAG_GRP  *)&FLAG_KEY,
